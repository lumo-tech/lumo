# copied from https://github.com/briancurtin/deprecation/blob/master/deprecation.py
#
...
def deprecated(deprecated_in=None, removed_in=None,
    ...
    if isinstance(removed_in, date):
        ...
        if (removed_in
                and current_version >= removed_in.split('.')):
            is_unsupported = True
        elif (deprecated_in
              and current_version >= deprecated_in.split('.')):
            is_deprecated = True
    else:
